...
async def predict_price(
    ...
    try:
        ...
        success = LokiLogger.send_log(
            level="INFO",
            message=f"Previsão realizada para {symbol}: {current_price:.2f} → {prediction:.2f} ({price_change_pct:+.2f}%)",
            extra_data={
                "symbol": symbol,
                "current_price": current_price,
                "predicted_price": prediction,
...
async def get_model_info():
    ...
    try:
        ...
        # Enviar log de previsão ao Loki
        success = LokiLogger.send_log(
            level="INFO",
            message=f"Model Info {"eth_price_predictor"}: {features:.2f} → {training_date:.2f} ({analysis:+.2f}%)",
            extra_data={
                "name": "eth_price_predictor",
                "features": features,
                "training_date": training_date,
